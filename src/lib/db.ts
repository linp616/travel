...
export const fileDb = {
  ...
  hotelRecommendation: {
    ...
    deleteMany: (where: { tripId: string }) => {
      const d = load()
      d.hotels = d.hotels.filter((h) => h.tripId !== where.tripId)
      save(d)
    }
  },
  transportRecommendation: {
    ...
    deleteMany: (where: { tripId: string }) => {
      const d = load()
      d.transports = d.transports.filter((t) => t.tripId !== where.tripId)
      save(d)
    }
  },
  itineraryDay: {
    ...
    deleteMany: (where: { tripId: string }) => {
      const d = load()
      d.itineraryDays = d.itineraryDays.filter((i) => i.tripId !== where.tripId)
      save(d)
    },
    findMany: (where: { tripId: string }, opts?: { orderBy?: { dayIndex: "asc" | "desc" } }) => {
      const d = load()
      let list = d.itineraryDays.filter((i) => i.tripId === where.tripId)
  ...
  xhsNoteSummary: {
    ...
    deleteMany: (where: { tripId: string }) => {
      const d = load()
      d.xhsNotes = d.xhsNotes.filter((n) => n.tripId !== where.tripId)
      save(d)
    }
  }
}
