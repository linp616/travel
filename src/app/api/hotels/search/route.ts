  ...
  try {
    ...
    if (tripId) {
      ...
      for (const h of hotels) {
        await prisma.hotelRecommendation.create({ id: randomBytes(12).toString("hex"), tripId, name: h.name, price: h.price, rating: h.rating, address: h.address, source: h.source, sourceLabel: h.sourceLabel, url: h.url, tag: h.tag, createdAt: getNow() })
      }
    }
